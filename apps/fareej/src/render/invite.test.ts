/* LIFTED FROM apps/luluaa/src/render/invite.test.ts, along with the
   component it guards. The QR path is generated code — the only way to know
   it is right is to rasterise it and decode it back. */
/* =========================================================================
...
describe("invite QR", () => {
  it("decodes back to the exact join link", () => {
    const url = "https://kaz6.netlify.app/games/fareej/?room=Y3HPK";
  ...
  it("survives the longest realistic link", () => {
    ...
    const url = "https://www.khalifaothman-portfolio.example.com/games/fareej/index.html?room=WXYZ9&utm_source=whatsapp";
    expect(decode(url)).toBe(url);
  });

  it("distinguishes one room from another", () => {
    const a = "https://kaz6.netlify.app/games/fareej/?room=AAAAA";
    const b = "https://kaz6.netlify.app/games/fareej/?room=BBBBB";
  ...
  it("keeps the quiet zone clear, or scanners won't lock on", () => {
    const { data, w, size } = rasterise("https://kaz6.netlify.app/games/fareej/?room=Y3HPK");
