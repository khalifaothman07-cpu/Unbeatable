/* =========================================================================
   ...
   -------------------------------------------------------------------------
   LIFTED FROM apps/luluaa/src/state/sync.ts. Only the table and function
   names differ. If you fix a bug in the seam here, fix it there too — the
   two files are meant to stay identical apart from those names.
   -------------------------------------------------------------------------
   LAZY BY DESIGN: nothing here runs while every seat is local.
   ...
   What travels is a SNAPSHOT of the mutable state. FAREEJ's board is fixed
   — forty spaces that never change — so unlike LU'LU'A there is no seed to
   rebuild from and nothing derived to leave out. What moves is who owns
   what, what is built, where everyone is standing, and how much they hold.
   ========================================================================= */
...
export async function closeRoom(code: string): Promise<boolean> {
  const c = getClient();
  if (!c) return false;
  const { data, error } = await c.rpc("fareej_close_room", { p_code: code });
...
export async function claimSeat(code: string, seat: number): Promise<Claims | null> {
  const c = getClient();
  if (!c) return null;
  const { data, error } = await c.rpc("fareej_claim_seat", { p_code: code, p_seat: seat });
